...
[`credentials`](#credentials)
  - The `credentials` event is fired when more credentials must be
    given to continue.
...
[`RFB.sendCredentials()`](#rfbsendcredentials)
  - Send credentials to server. Should be called after the
    [`credentials`](#credentials) event has fired.
...
The `credentials` event is fired when the server requests more
